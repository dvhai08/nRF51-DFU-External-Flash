...
static dfu_state_t                  m_dfu_state_remit;                /**< Current DFU state. */
static uint32_t                     m_image_size_remit;               /**< Size of the image that will be transmitted. */
...
static pstorage_handle_t            m_storage_handle_app_remit;       /**< Pstorage handle for the application area (bank 0). Bank used when updating a SoftDevice w/wo bootloader. Handle also used when swapping received application from bank 1 to bank 0. */
static pstorage_handle_t          * mp_storage_handle_active_remit;   /**< Pointer to the pstorage handle for the active bank for receiving of data packets. */

static dfu_callback_t               m_data_pkt_cb_remit;              /**< Callback from DFU Bank module for notification of asynchronous operation such as flash prepare. */
static dfu_bank_func_t              m_functions_remit;                /**< Structure holding operations for the selected update process. */

static uint32_t 										m_data_received_remit;

h_update_status_t h_dfu_update_status = DFU_UPDATE_IDLE;
h_update_status_t h_dfu_update_status_store;
h_pstorage_status_t h_pstorage_status = PSTORAGE_BUSY;
 ...
 */
static void pstorage_callback_handler_remit(pstorage_handle_t * p_handle,
                                      uint8_t             op_code,
                                      uint32_t            result,
                                      uint8_t           * p_data,
                                      uint32_t            data_len)
{
		h_pstorage_status = PSTORAGE_IDLE;
...
void dfu_prepare_func_app_erase_remit(uint32_t image_size)
{
    ...
    mp_storage_handle_active_remit = &m_storage_handle_app_remit;

    // Doing a SoftDevice update thus current application must be cleared to ensure enough space
    // for new SoftDevice.
    h_pstorage_status = PSTORAGE_BUSY;
    err_code    = pstorage_clear(&m_storage_handle_app_remit, image_size);
...
{
    uint32_t                err_code;
    pstorage_module_param_t storage_module_param = {.cb = pstorage_callback_handler_remit};

    err_code = pstorage_register(&storage_module_param, &m_storage_handle_app_remit);    
    m_storage_handle_app_remit.block_id  = DFU_BANK_0_REGION_START;
    return NRF_SUCCESS;
}
...
{
	if(m_dfu_state_remit == DFU_STATE_RDY)
		return 1;
	return 0;
}
...
uint32_t dfu_data_pkt_handle_remit(uint32_t * p_data_packet,uint32_t packet_length)
{
    ...
    VERIFY_PARAM_NOT_NULL(p_data_packet);

    // Check pointer alignment.
    if (!is_word_aligned(p_data_packet))
    {
		...
		h_pstorage_status = PSTORAGE_BUSY;
		err_code = pstorage_store(mp_storage_handle_active_remit,
                                          (uint8_t *)p_data_packet,
                                          packet_length,
                                          m_data_received_remit);
		VERIFY_SUCCESS(err_code);
		m_data_received_remit += packet_length;            
